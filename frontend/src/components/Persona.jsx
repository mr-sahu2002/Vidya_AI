...
const Persona = () => {
  ...
  const handleSubmit = () => {
    if (Object.keys(answers).length !== questions.length) {
      ...
      setError(false);
      // You could save the persona/answers to a backend here
      navigate("/login");
    }
  };
  ...
  return (
    <div className="signup-bg">
      <div className="signup-card" style={{ maxHeight: "90vh", overflowY: "auto" }}>
        <h2 style={{ textAlign: "center" }}>Learning Persona</h2>
        <form className="signup-form" onSubmit={(e) => e.preventDefault()}>
          {questions.map((q) => (
            <div key={q.id} className="persona-question">
              <p className="persona-question-text">
                {q.id}. {q.text}
              </p>
              <div className="persona-options-group">
                {Object.entries(q.options).map(([key, label]) => (
                  <label key={key} className="persona-option">
                    <input
                      type="radio"
                      name={`question-${q.id}`}
                      value={key}
          ...
          {error && (
            <div className="persona-error">
              Looks like you haven't answered all the questions. Please fill all the answers.
            </div>
          )}
        </form>
      </div>
